  ...
  try {
    ...
    if (product.type === "free") {
      return res.status(200).json({
        ...
        installation: "Load the JSON file into your context to activate the skill."
      });
    }
    ...
    if (payment_method === "stripe") {
      if (!process.env.STRIPE_SECRET_KEY) {
        return res.status(500).json({
          ...
          message: "Stripe is not set up. Please contact support@botbay.io"
        });
      }
    ...
    if (payment_method === "crypto") {
      ...
      return res.status(200).json({
        ...
        payback_days: roi_data.payback_days,
        annual_savings: roi_data.annual_savings,
        message: "Send exact amount to the crypto address. Skill delivered automatically upon confirmation.",
        instructions: [
        ...
        warning: "Send ONLY " + crypto_currency.toUpperCase() + " to this address. Other tokens will be lost.",
    ...
    return res.status(500).json({
      ...
      support: "If this persists, contact support@botbay.io"
    });
  }
}
...
function calculateROI(product_id, price_usd) {
  ...
  const roi_info = roi_map[product_id] || { monthly_value: 100, payback_hours: 24 };
